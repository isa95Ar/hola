import{t as e}from"./plan-DNSMCIpP.js";var t=e.plan_de_trabajo_facttic_2026_2027,n={A:{hex:`#F1DA1C`,text:`text-brand-yellow`,border:`border-brand-yellow`,dot:`bg-brand-yellow`,cardBorder:`hover:border-brand-yellow`},B:{hex:`#68DE94`,text:`text-brand-green`,border:`border-brand-green`,dot:`bg-brand-green`,cardBorder:`hover:border-brand-green`},C:{hex:`#FF6853`,text:`text-brand-orange`,border:`border-brand-orange`,dot:`bg-brand-orange`,cardBorder:`hover:border-brand-orange`}},r=new URLSearchParams(window.location.search).get(`eje`)?.toUpperCase(),i=t.find(e=>e.eje.startsWith(`EJE ${r}`)),a=document.getElementById(`eje-content`);if(!i)a.innerHTML=`<p class="text-brand-gray text-center">Eje no encontrado.</p>`;else{document.title=`${i.eje} — Plan Facttic`;let e=n[r]??n.A,o=t.indexOf(i),s=t[o-1]??null,c=t[o+1]??null,l=s?.eje.split(` `)[1],u=c?.eje.split(` `)[1],d=i.objetivos.map(t=>`
  ...
  `;let f=document.getElementById(`modal`),p=document.getElementById(`modal-bar`),m=document.getElementById(`modal-id`),h=document.getElementById(`modal-title`),g=document.getElementById(`modal-list`),_=document.getElementById(`modal-close`);function v(t){m.textContent=t.id,m.className=`text-xs font-mono font-bold ${e.text}`,h.textContent=t.objetivo,p.style.background=e.hex,t.accionables.length?g.innerHTML=t.accionables.map((t,n)=>{let r=t.plazo?`<span class="inline-flex items-center gap-1.5 text-xs rounded-full px-2.5 py-0.5 border" style="color:${e.hex}; border-color:${e.hex}40; background:${e.hex}12">
      ...
      `,f.style.display=`flex`,document.body.style.overflow=`hidden`,g.querySelectorAll(`li`).forEach(e=>{requestAnimationFrame(()=>{e.style.opacity=`1`})})}function y(){f.style.display=`none`,document.body.style.overflow=``}a.querySelectorAll(`button[data-id]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=i.objetivos.find(t=>t.id===e.dataset.id);t&&v(t)})}),_.addEventListener(`click`,y),backdrop.addEventListener(`click`,y),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&y()})}